...
>
> - **Build the resolved/frozen maps only from annotations that have NOT been voided.** A `voids_annotation` row names an earlier row by clause + timestamp; drop that earlier row before doing anything else. Append-only means a wrong annotation stays on disk forever, so honoring it forever is a choice, not a constraint.
> - **`resolves_clause` windows the clause it literally names, plus that clause's aliases, never a parent's children.** Fixing one sub-question does not age out its siblings' debt. If a decomposition genuinely resolves a whole family, write one `resolves_clause` row per sub-clause and say so; the extra rows are the point, because each one is a claim someone can check.
