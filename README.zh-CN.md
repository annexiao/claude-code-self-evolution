...
> <details>
> <summary><sub><b>2026-08-16:</b> 两个队列都有了出口：defer 的候选和没人用的 memory 可以自然老化出队，不再只进不出。</sub></summary>
>
> - **`defer` 不再是单向门：** 一个候选被 defer 过 3 次以上、首次出现超过 3 个月、且始终没有第二个实例，就判为「证据不足」并归档。次数和时间必须同时满足：只看次数等于假设了一个固定的运行频率，只看时间又可能淘汰掉只被判过一次的东西。
> - **memory 索引拿到同一个出口：** 一条 memory 超过 5 个月、从未被召回、且没有第二条 memory 指向它，同样归档。**framework 类豁免**，理由就是准入那一侧已经给过的：可迁移的框架第一次说出来就有价值，所以不能用一个复现形状的判据把它淘汰掉。
> - **归档不是删除：** 两道门都只是把文件移进 `.processed/`，随时可以捞回来。
> - **没有传感器的条件会明说：** memory 那道门的「从未被召回」需要一个召回计数，而它可能不存在。规格没有把「没有记录」读成「没有发生」，而是要求先去查，在计数器建起来之前把这个条件挂起。
>
> </details>
>
> <details>
> <summary><sub><b>2026-07-25:</b> 执行债（enforcement debt）三处加强：查重覆盖更广、强化欠债原因的记录、已修复的旧债淡出计数。</sub></summary>
>
> - **查重更全：** 「这条已经覆盖了吗」这一步，现在除了常驻规则以外，还会去磁盘通读整个技能库、路径规则和经验库。
> - **强化欠债原因的记录：** 执行债的 triage 从结构上强化了对「欠债原因」的记录，并接入了实际数债的操作步骤。
> - **旧债随修复淡出：** 某条规则的根因一旦被修复，它修复前累积的旧债就不再计入 graduation（毕业）阈值。
>
> </details>
>
> <details>
> <summary><sub><b>2026-07-20:</b> 大型审批渲染成可读的页面、验证会问「这个检查有没有可能失败」、确认门按爆炸半径来缩放。</sub></summary>
>
> - **审批渲染成网页页面，方便阅读。** 超过几十条待写项时，页面生成「一句话摘要」和可展开的「背后的完整证据」。如果是小范围的审批，那么聊天框按照七行一批地提供 proposal。见 [docs/REVIEW-BOARD.md](docs/REVIEW-BOARD.md)。
> - **「下结论前先验证」现在以两种失败形态 + 四问（层、范围、时机、裁判）领起。** 反复出现的那种失败是：检查跑了，但它结构上不可能得出相反结论；规则把这种形态摆在最前，钩子也能抓到它。
> - **确认门现在按爆炸半径和可逆性来缩放，不再打断「一条命令就能回滚」的自有部署。** 这是 triage 模型第一次「收窄」一条规则，而不是「加硬」。
>
> </details>
>
> <details>
> <summary><sub><b>2026-07-15:</b> enforcement 模型学会了先诊断一条规则为什么失效，再决定怎么修；plan 表也学会了说清楚它到底在提议什么。</sub></summary>
>
...
>
> <details>
> <summary><sub><b>2026-06-27:</b> 框架不再需要重复出现才算数。</sub></summary>
>
> - **复发门槛现在只适用于纠正。** 一条行为纠正仍然要出现两次才成为规则，因为一次可能只是失手。可迁移的框架不需要：它是一个透镜，不是一个不稳定的模式，第一次被说清楚就值得留下。等一个可能永远不来的第二次，等于静默丢掉这个系统最该捕获的信号。
> - **已被覆盖的候选不再干扰提升率。** 它们是归档不是提升，所以整个退出计算基数。规则提升和记忆提升现在也分开计量，因为一批框架**本来就该**变成记忆。
>
> </details>
