...
> <details>
> <summary><sub><b>2026-07-20:</b> 让监督机制更精准。大型审批要让人真正看得懂；验证不能只看「有没有检查」，还要看检查是否有效；确认机制也不能不分风险大小地一刀切。</sub></summary>
>
> - **大批量运行现在把审批界面渲染成一个页面，而不是聊天里的表格。** 超过几十条提议之后，表格没有好的设置：写全了读不完，压缩了你批准的就是一个分类标签而不是一条提议。页面把这个矛盾溶掉了，因为一行摘要和它背后的完整证据可以同时存在，由你决定看哪个。`/evolve` 同时给聊天回退路径加了一条朴素的尺寸规则：一个复查界面最多约七行，更长就分批。见 [docs/REVIEW-BOARD.md](docs/REVIEW-BOARD.md)。
> - **verify-before-claiming 规则被拆开了，因为它的 enforcement hook 一直抓的是错的那一半。** 那个 hook 在 agent 声称某事已完成或已上线却拿不出依据时触发，它是有效的。但反复复发的失效是另一个形状：检查**做了**，只是那一下不可能得出相反的结论。这条原则本来就写在规则里，作为它的最后一行，压在二十多条累积的例子底下。规则现在以两种形状和一个四问测试开头（对你自己的检查问：层、范围、时机、裁判），累积的例子降级为举证。hook 在两个层面都做了扩展，因为第二种形状的消息此前根本到不了它的判断层。
> - **一条规则被收窄，而不是加硬，这是第一次。** triage 模型一直有三种结果，而只有两种真正触发过。一个为对外动作设计的确认门，一直在所有者自己的、一条命令就能回滚的部署上触发，加了一个来回却没有减少任何风险。它现在按爆炸半径和可逆性校准，这正是一个 over-scoping 诊断本该产出的东西。
>
> </details>
>
> <details>
