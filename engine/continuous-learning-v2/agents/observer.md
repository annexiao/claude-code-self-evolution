...
> **Local customization (the user, 2026-05-22):** Time-based decay is handled
> deterministically by `~/.claude/scripts/apply-instinct-decay.py` on a
> weekly launchd schedule (rate: -0.005/week). **Do NOT apply decay
> yourself**, your job is only the evidence-driven +0.05 / -0.1 above.
> Original upstream rule was "-0.02 per week without observation (decay)".
