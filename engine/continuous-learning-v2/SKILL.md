...
Before forming or evolving instincts, scan **all** ADRs in `docs/decisions/` of the current project, not only those marked `Accepted`. ADRs marked `Superseded by ...` carry the constraints and rejected alternatives that shaped the current state, and those constraints often still apply. Treating only "current" ADRs as the project's truth misses ~half of the decision history. The audit trail (Accepted + Superseded) is the value, not just the latest answer; instincts learned without it will re-suggest patterns the project has already tried and rejected.
